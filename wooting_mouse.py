...
class Mouse:
    ...
    def __init__(self, state, device, config):
        cap = {
            evdev.ecodes.ecodes["EV_REL"]: list(
                config["mouse"]["ev_abs_mouse_write_map"].values()
            ),
            evdev.ecodes.ecodes["EV_KEY"]: list(
                config["gamepad"]["ev_key_mouse_write_map"].values()
            ),
        }

        for value in config["gamepad"]["ev_abs_mouse_write_map"].values():
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            for from_event, to_event in config["mouse"][
